...
label start:
    "Начало конца"
    # здесь вставить небольшую историю, где он сейчас находится

    scene bg saloon
    with Dissolve(.5)

    # какая-то вводная хуйня
    ...
    label toilet_single:
        ...
        "{cps=25}Дерьмо?{/cps}"
        "{cps=25}Дерьмо.{/cps}"
        "{cps=25}Если бы мне в академии сказали, что я буду работать вот так, я бы лучше даже школу не заканчивал.{/cps}"
        "{cps=25}Вот Шура, как там его... Забыл фамилию. Так тот после десятого в колледж ускакал, и вроде не жалуется.{/cps}"
        "{cps=25}Или он сидит? Не помню уже ничего.{/cps}"
        "{cps=25}Не надо было в то дело лезть, совсем забыл, в какой стране родился и живу.{/cps}"
        "{cps=25}Ну порешил полковник женушку, зачем было лезть. Говорили мне в Следственном Комитете, не надо браться, самому же попадет.{/cps}"
        "{cps=25}Когда у человека много власти, он и жену убить может, и никто ему ничего не скажет. Жизнь так устроена. Сильный бьет слабого, как король бьет валета.{/cps}"
        "{cps=25}Правильно ли я поступил?{/cps}"

        scene bg stairs
        with Dissolve(.5)

        "{cps=25}Правильно, конечно. Ублюдок должен быть наказан, вот как я считаю. Да только наказан в итоге я. Прозябаю теперь в какой-то дыре, тут даже гостиницы нет.{/cps}"
        "{cps=25}А полковнику – хоть бы хрен: ворует как воровал, блядствует как блядствовал. Хорошо хоть не замочил, в теории мог бы и это. Весь суд купил, как бублики на базаре.{/cps}"

        scene bg toilet
        with Dissolve(.5)

        "{cps=25}Головой в сортире – какая дрянь. Жизнь человек и так не выбирает, так некоторым еще и в смерти достается. Не заслужил инсульт, что ли?{/cps}"
    ...
    label question_single:
        ...
        Boris "{cps=25}В этом городе только один закон, детектив.{/cps}{nw}"
        with flashbulb
